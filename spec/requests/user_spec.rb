describe 'GET /users' do
  it 'should send a list of users' do
    3.times { |_x|  FactoryGirl.create(:user) }
...
describe 'POST /users' do
  ...
  it 'should return a success message' do
    ...
    expect(json.key?('status')).to eq(true)
    expect(json['status']).to eq('created')
    expect(response.status).to eq(201)
    expect(json.key?('errors')).to eq(false)
  end
  ...
  it 'should return error messages on failure' do
    ...
    errors = json['errors']
    expect(errors.key?('email')).to eq(true)
    expect(errors.key?('username')).to eq(true)
    expect(errors.key?('age')).to eq(true)
    expect(errors.key?('password')).to eq(true)
  end
end
...
describe 'GET /verify' do
  ...
  it 'should return a fresh token' do
    url = get_verify_link(user)
    get url
    expect(json.key?('jwt')).to eq(true)
  end
